...
export default function ReportLotModal({
  parkingLot,
  onClose,
  onReported,
}: ReportLotModalProps) {
  ...
  async function handleFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setError(`${getErrorMessage(err)} You can still submit the report without a photo.`);
    } finally {
      setUploading(false);
    }
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (locating) {
      setError('Still waiting for location data...');
      return;
    }
    
    if (!location) {
      setError('Location access is required to prevent spam reports.');
      return;
    }
    ...
    setLoading(true);
    try {
      await reportLotIssue(parkingLot.id, {
        issueType,
      ...
      notifySuccess('Report filed. We have passed it on to the parking owner and our admins.');
      onReported?.();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Report an issue with this parking lot"
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-5" noValidate>
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            ...
            <Button
              ...
              disabled={uploading || locating || !location}
              fullWidth={false}
            >
              {loading ? 'Submitting...' : locating ? 'Getting location...' : 'Submit report'}
            </Button>
          </div>
        </form>
      </div>
